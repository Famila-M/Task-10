let api_key = "9f9aec9f66886782da6c14ecd12ea72b";
...
const getWeather = (lat, lon) => {
    return new Promise(async(resolve, reject) => {
        try {
            ...
            const response = await fetch('https://api.openweathermap.org/data/2.5/weather?lat=' + lat + '&lon=' + lon + '&appid=9f9aec9f66886782da6c14ecd12ea72b')
            const data = await response.json();
            resolve(data);
        } catch (e) {
            reject(e);
...
document.body.style.backgroundColor = "#50686e";
...
function createCountryCard(data, i) {
    ...
    cardBody.style.backgroundImage = 'linear-gradient(to right, #ccffff 0%, #669999 100%)';
